...
export default function NewPasswordPage({ params }: { params: { locale: Locale } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Валидация
    if (!email || !code) {
      setError('Данные для сброса пароля не найдены. Начните процесс заново.')
      setIsLoading(false)
      return
    }
  ...
  // Если нет данных для сброса, показываем ошибку
  if (!email || !code) {
    return (
            ...
            <div className="text-center space-y-4">
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-red-700 text-sm">
                  Данные для сброса пароля не найдены. Начните процесс заново.
                </p>
              </div>
              <Link href={`/${locale}/auth?mode=reset`}>
      ...
      <Card className="w-full max-w-sm sm:max-w-md">
        <CardHeader className="text-center pb-6">
          ...
          <CardDescription className="text-sm sm:text-base">
            Создайте надежный пароль для<br/>
            <strong>{email}</strong>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 px-4 sm:px-6 pb-6">
